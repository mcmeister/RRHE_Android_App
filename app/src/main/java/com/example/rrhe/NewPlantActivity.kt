...
class NewPlantActivity : AppCompatActivity(), PhotoManager.PlantBinding {
    ...
    // Implement the logic to upload photos after StockID update
    private fun uploadPhotosAfterStockIDUpdate(stockID: Int) {
    ...
    private fun handlePlantSave() {
        lifecycleScope.launch {
            ...
            // Check if the StockID was updated from the temporary one
            currentPlant?.StockID?.let { stockID ->
                if (stockID > 0) {
                    // StockID has been updated, now upload photos
                    uploadPhotosAfterStockIDUpdate(stockID)
                }
            }
        }
    }
