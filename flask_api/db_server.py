...
def update_rrhe():
    ...
    stamp = data.get('Stamp')  # This is already being extracted
    family = data.get('Family')
    species = data.get('Species')
    subspecies = data.get('Subspecies')
    name_concat = data.get('NameConcat')
    ...
    thai_name = data.get('ThaiName')
    m_id = data.get('M_ID')
    f_id = data.get('F_ID')
    plant_status = data.get('PlantStatus')
    status_note = data.get('StatusNote')
    mother = data.get('Mother')
    website = data.get('Website')
    table_name = data.get('TableName')
    tray_size = data.get('TraySize')
    grams = data.get('Grams')
    total_value = data.get('TotalValue')
    usd = data.get('USD')
    eur = data.get('EUR')
    planted_start = data.get('PlantedStart')
    planted_end = data.get('PlantedEnd')
    pollinate_date = data.get('PollinateDate')
    seeds_planted = data.get('SeedsPlanted')
    seeds_harvest = data.get('SeedsHarvest')
    photo_link1 = data.get('PhotoLink1')
    photo_link2 = data.get('PhotoLink2')
    photo_link3 = data.get('PhotoLink3')
    photo_link4 = data.get('PhotoLink4')

    # Validate essential fields
    if not stock_id or not stamp:
        return jsonify({'error': 'StockID and Stamp are required'}), 400

    connection = get_db_connection()
    try:
        with connection.cursor() as cursor:
            # Get the current Stamp for the plant
            cursor.execute("SELECT Stamp FROM stock WHERE StockID = %s", (stock_id,))
            current_stamp = cursor.fetchone()
            if current_stamp and current_stamp[0] >= stamp:
            ...
            cursor.execute("""
                ...
                WHERE StockID=%s
            """, (family, species, subspecies, name_concat, stock_qty, stock_price, purchase_price, plant_description, thai_name, m_id, f_id, plant_status, 
                  status_note, mother, website, table_name, tray_size, grams, total_value, usd, eur, planted_start, planted_end, pollinate_date, seeds_planted, seeds_harvest,
                  photo_link1, photo_link2, photo_link3, photo_link4, stamp, stock_id))
